...
class Generator(nn.Module):
    ...
    def init_shared_optim(self):
        p_dict = dict(self.named_parameters())
        ds = 0
        tot=0
        for k in p_dict.keys():
            tmp = torch.prod(torch.tensor(p_dict[k].size(),dtype=torch.long))
            if 'shared_weight' not in k:
                ds += tmp
            tot +=tmp
        print(f'ratio trainable params = {float(ds)/float(tot)}')

        optim_params = self.get_trainable_params()
        self.optim = optim.Adam(params=optim_params, lr=self.lr,
                                                 betas=(self.B1, self.B2), weight_decay=0,
                                                 eps=self.adam_eps)
    # Initialize
    ...
    def get_trainable_params(self):
        par = dict(self.named_parameters())
        p_list = []
        with torch.no_grad():
            for k in par.keys():
                if not 'shared_weight' in k:
                    p_list+= [par[k]]
        return p_list
    
    def init_shared_layers(self,learn_ratio):
        self.init_shared_conv_layers(learn_ratio)
        self.init_shared_lin_layers(learn_ratio)
        
    def init_shared_conv_layers(self,learn_ratio):
        with torch.no_grad():
            ...
            if learn_ratio < 1:
                # split modulation layers
                for k in par.keys():
                    if 'conv' in k and 'weight' in k:
                        shared_w_name = k[:-6] + 'shared_weight'
                        w = rgetattr(self, k)
                        out_channel = w.size(dim)
                        ndif_params = int(learn_ratio*out_channel)
                        l = torch.split(w,[ndif_params,out_channel-ndif_params],dim=dim)
                        rsetattr(self,k,nn.Parameter(l[0].clone()).cuda())
                        rsetattr(self,shared_w_name,nn.Parameter(l[1].clone()).cuda())
    def init_shared_lin_layers(self,learn_ratio):
        with torch.no_grad():
            learn_ratio = learn_ratio
            par = dict(self.named_parameters())
            dim = 0
