...
class SN(object):
  ...
  def W_(self):
    # concat width-wise shared weights
    if (not hasattr(self, 'shared_weight')) or self.shared_weight==None:
        weight = self.weight
    else:
        weight = torch.cat([self.weight,self.shared_weight],dim=1)
        
    W_mat = weight.view(self.weight.size(0), -1)
    if self.transpose:
      W_mat = W_mat.t()
...
class SharedLinear(nn.Linear):
  ...
  def forward(self, x):
    if self.shared_weight is not None:
        weight = torch.cat([self.weight,self.shared_weight],dim=0)
    else:
        weight = self.weight
